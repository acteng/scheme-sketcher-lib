<script>import { mode } from "../stores";
import { layerId } from "../../maplibre";
...
$:
  if ($map && $imgSrc && $mode.mode == "set-image" && topLeft.lng == 0 && topLeft.lat == 0) {
    let bounds = $map.getBounds();
    topLeft.lng = bounds.getWest() + 0.4 * (bounds.getEast() - bounds.getWest());
    bottomRight.lng = bounds.getWest() + 0.6 * (bounds.getEast() - bounds.getWest());
    topLeft.lat = bounds.getNorth() + 0.4 * (bounds.getSouth() - bounds.getNorth());
    bottomRight.lat = bounds.getNorth() + 0.6 * (bounds.getSouth() - bounds.getNorth());
  }
</script>
